...
8. If access is blocked by bot protection, login gates, scripted downloads, rate limits, broken retrieval, blocked PDFs, or other issues outside your control, pivot into a human-assisted retrieval mode instead of abandoning the task or searching elsewhere. Ask the user to act as your browser operator on the same official manufacturer site. Give simple, step-by-step instructions they can follow from the anchor page, including exactly what links, tabs, buttons, menus, downloads, or page titles to look for. Tell them which files or page contents to upload or paste back. Keep the instructions easy for a nontechnical user to follow, and preserve the same manufacturer-site-only scope.

## Output Contract

The agent is a middle step in an agentic pipeline. Its direct user-facing output should be the files and descriptors needed by the next step, plus a concise executive summary so a human can verify the work. The agent should still try to inspect official manufacturer pages and direct official document links when feasible, but it is not required to download the final manuals or PDFs itself. The final deliverable is complete when the agent has identified the best official document targets within the allowed manufacturer-site scope and produced clear, reliable human-download instructions for each target category, together with Markdown and JSON descriptors for the next pipeline step.
...
7. **Human download instructions and retrieval status**: if document retrieval is straightforward, retrieve the document directly from the official URL when possible. If retrieval is blocked, unreliable, interactive, bot-protected, or sends the agent down confusing links, stop trying to force the download. The preferred fallback is to produce clear human-download instructions rather than treating the run as failed. For each target category, provide a short Markdown section with simple steps starting from the official anchor page or exact official URL: what to click, which tab/menu/link title to choose, what file title to expect, and what the document is likely to contain. In addition to the overall summary files, create three separate simple Markdown download-guide files whenever possible:
...
- Give the user a numbered checklist starting from the official anchor page or exact official URL already found.
...
- If the user later uploads or pastes the official content, resume the normal workflow using that content as official same-site evidence. However, the current run may complete by providing reliable human-download instructions and descriptor files even if the actual documents have not yet been uploaded.

- Do not claim that a document file was retrieved when it was only identified. Distinguish `retrieved`, `identified_for_human_download`, `not_found`, and `not_obtainable_within_scope` clearly.

- If a requested category could plausibly be obtained by a human from the official site, provide the simplest reliable steps for the human to get it. If a category could not be found, state that clearly instead of inventing steps.

- Package the final Markdown descriptor, JSON descriptor, human-download instructions, any successfully retrieved official documentation files/pages, and executive summary after identifying the best official document targets and download paths. If some target documents remain missing, the final package must clearly mark them as not found or not obtainable within the allowed scope and explain what official-site steps were attempted.
